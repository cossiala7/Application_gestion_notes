pipeline {
    ...
    environment {
        IMAGE_NAME = "cossiala7/gestion_notes:latest" // ← change "tonusername"
    ...
    stages {
        stage('Cloner le projet') {
            steps {
                git 'https://github.com/cossiala7/Application_gestion_notes.git' // ← remplace par ton dépôt Git
            }
        }
        ...
        stage('Push vers Docker Hub') {
            steps {
                withCredentials([usernamePassword(
                    credentialsId: 'delatchaille', // ← à créer dans Jenkins
    ...
    post {
        ...
        success {
            // Envoi d'un email en cas de succès
            emailext (
